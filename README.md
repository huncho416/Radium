...
### 🏷️ Rank-Based Nametags
- **Custom Templates**: MiniMessage-formatted nametags for each rank
- **Live Updates**: Real-time nametag changes on rank/vanish status updates
- **Vanish Integration**: Smart visibility based on rank weight and permissions
- **Temporary Overrides**: API support for temporary nametag changes
- **Backend Sync**: Redis-based communication with Minestom servers
- **Hot Reload**: `/nametag reload` command for configuration updates
...
Radium provides rank-based custom nametags using MSNameTags for Minestom backend servers. Nametags are configured on the Velocity proxy and synchronized to backends via Redis.

### Configuration

Configure nametags in `plugins/radium/nametags.yml`:

```yaml
enabled: true
default-template: "<gray><username></gray>"

# Rank-specific templates (supports MiniMessage format)
ranks:
  OWNER:
    template: "<#ff355e><b>OWNER</b></#ff355e> <white><username>"
    show-to-self: true
    show-when-vanished: false
  ADMIN:
    template: "<red><b>ADMIN</b></red> <white><username>"
    show-to-self: true
    show-when-vanished: false
  MOD:
    template: "<green><b>MOD</b></green> <white><username>"
    show-to-self: true
    show-when-vanished: false
...
visibility:
  ...
  weight-gating:
    enabled: true
    fallback-template: "<dark_gray>•</dark_gray> <gray><username>"
...
Available placeholders in nametag templates:
- `<username>` - Player's username
- `<rank_name>` - Player's primary rank name
- `<rank_color>` - Rank color code
- `<weight>` - Rank weight value
- `<server>` - Current server name

### Vanish & Weight Behavior

**Visibility Rules**:
- **Not Vanished**: Nametag visible to all players
- **Vanished + `radium.vanish.see`**: Visible to players with permission
- **Vanished + Weight Gating**: Visible only to viewers with equal/higher rank weight
- **Self View**: Controlled by `show-to-self` setting per rank

**Example**: Admin (weight 100) vanished → visible to other Admins (weight 100+), invisible to Mods (weight 50)

### Commands & Permissions

| Command | Permission | Description |
|---------|------------|-------------|
| `/nametag reload` | `radium.nametag.reload` | Reload configuration |
| `/nametag preview <player> <template>` | `radium.nametag.preview` | Preview template (30s) |
| `/nametag refresh [player]` | `radium.nametag.refresh` | Refresh nametags |
| `/nametag info [player]` | `radium.nametag.info` | Show nametag info |

### Redis Channels

Nametag system uses these Redis channels for backend communication:
- `radium:nametag:update` - Nametag template updates
- `radium:player:vanish` - Vanish status changes
- `radium:player:profile:response` - Profile updates
- `radium:grant:added` / `radium:grant:removed` - Rank changes

### API Integration

```kotlin
// Temporary template override
NametagAPI.setTemporaryTemplate(playerId, "<rainbow>SPECIAL</rainbow> <white><username>", Duration.ofMinutes(5))

// Refresh specific player
NametagAPI.refresh(playerId)
