...
@Command("friend")
class Friend(private val mythLink: MythLink) {
    ...
    suspend fun friendAdd(actor: Player, target: String) {
        // Get profiles
        val actorProfile = mythLink.connectionHandler.getPlayerProfile(actor.uniqueId) ?: return
        val targetProfile = mythLink.connectionHandler.findPlayerProfile(target)
        ...
        when (actorProfile.addFriend(targetProfile)) {
            Profile.FriendResult.SENT -> {
                ...
                val targetPlayer = mythLink.server.getPlayer(targetProfile.uuid)
    ...
    suspend fun friendList(actor: Player) {
        ...
        actorProfile.updateFriendsLastSeenFromRedis(mythLink)
        ...
        friends.forEach { friendId ->
            val friendProfile = mythLink.connectionHandler.findPlayerProfile(friendId.toString())
            if (friendProfile != null) {
                val friendOptional = mythLink.server.getPlayer(friendId)
        ...
        if (incomingRequests.isEmpty() && outgoingRequests.isEmpty()) {
            ...
            if (incomingRequests.isNotEmpty()) {
                ...
                incomingRequests.forEach { requesterId ->
                    val requesterProfile = mythLink.connectionHandler.findPlayerProfile(requesterId.toString())
            ...
            if (outgoingRequests.isNotEmpty()) {
                ...
                outgoingRequests.forEach { targetId ->
                    val targetProfile = mythLink.connectionHandler.findPlayerProfile(targetId.toString())
                    if (targetProfile != null) {
